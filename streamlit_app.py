...
def predict_npz(predictor: ModelPredictor, npz_path: Path, gloss_map, cat_map) -> dict:
    from evaluation.prediction.static_jz_guard import apply_numbers_letters_guards
    ...
    if raw.get("static_jz_demoted") or raw.get("letter_a_preferred"):
        demoted = gloss_map.get(int(raw["demoted_from"]), raw["demoted_from"])
        reason = raw.get("demote_reason") or "handshape mismatch"
        out["note"] = f"{reason} — demoted {demoted} → {out['gloss']}."
    return out
